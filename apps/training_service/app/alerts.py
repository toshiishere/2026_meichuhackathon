"""Fall alerting: a fall followed by stillness, judged on the source clock.

Every time is a source time — seconds along the live capture or the *recording*
being replayed — so a 4x replay raises exactly the same alerts, at the same
points of the recording, as playing it back at 1x would.
"""
...
def notify(event, detail, timeout=5):
    """Post to the alert bot; never let a notification failure stop inference."""
    url = os.getenv("NOTIFIER_URL", "http://dc-bot:8003")
    response = httpx.post(
        f"{url}/alert", json=dict(event=event, detail=detail), timeout=timeout
    )
